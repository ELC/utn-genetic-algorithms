...
class Population():
    ...
    def _cross_over_1_point(self, father1, father2):
        ...
        father1_gene = father1.get_gene_string()
        father2_gene = father2.get_gene_string()
        child1_gene = [-1 for _ in range(length)]
        child2_gene = [-1 for _ in range(length)]

        index = 0
        aux = father1_gene[index]

        while aux not in child1_gene:
            child1_gene[index] = aux
            aux = father2_gene[index]
            index = father1_gene.index(aux)
        
        for i, j in enumerate(child1_gene):
            if j == -1:
                child1_gene[i] = father2_gene[i]
                child2_gene[i] = father1_gene[i]
            else:
                child2_gene[i] = father2_gene[i]

        child1 = Chromosome(child1_gene)
        child2 = Chromosome(child2_gene)
...
if __name__ != "__main__":
    ...
    from exercise3.entities.chromosome import Chromosome
