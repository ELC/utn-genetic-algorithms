...
from exercise5.util.graphic import graphics
import exercise5.population as Population
import exercise5.controller
...
class Report():
    ...
    def write_csv(cls, df, execution_time):
        name = str(FileManager.get_settings_id())[:5]
        filename = "exercise5/results/" + name + '.csv'
    ...
    def write_excel(cls, df, execution_time):
        sheet_name = str(FileManager.get_settings_id())[1:5]
        filename = 'exercise5/results/resultados.xlsx'
    ...
    @classmethod
    def solution_report(cls, objects, total_vol):
        """Show the final solution infered from the generations"""
        data = FileManager.load_results()
        last_population = data[-1]
        objects = FileManager.load_settings()["objects"]
        total_vol = FileManager.load_settings()["total_vol"]
        fittest = Population.get_fittest_chromosome(last_population, objects, total_vol)
        return Population.get_price(fittest, objects)
    ...
    def best_gene():
        ...
        total_vol = FileManager.load_settings()["total_vol"]
        fittest = Population.get_fittest_chromosome(last_population, objects, total_vol)
        return Population.calc_indexes(fittest), Population.get_volume(fittest, objects)

    @classmethod
    def _get_array_data(cls, data):
        objects = FileManager.load_settings()["objects"]
        total_vol = FileManager.load_settings()["total_vol"]
        maximums = [cls.get_maximum(population, objects, total_vol) for population in data]
        minimums = [cls.get_minimum(population, objects, total_vol) for population in data]
        averages = [cls.get_average(population, objects, total_vol) for population in data]
        ranges = [cls.get_range(population, objects, total_vol) for population in data]
        total = [cls.get_sum(population, objects, total_vol) for population in data]
        chromosome = [Population.get_fittest_chromosome(population, objects, total_vol) for population in data]
        stationary = cls._get_stationary(data, objects, total_vol)
    ...
    @staticmethod
    def get_maximum(population, objects, total_vol):
        return max(Population.get_targets(population, objects, total_vol))

    @staticmethod
    def get_minimum(population, objects, total_vol):
        return min(Population.get_targets(population, objects, total_vol))

    @staticmethod
    def get_sum(population, objects, total_vol):
        return sum(Population.get_targets(population, objects, total_vol))

    @classmethod
    def get_average(cls, population, objects, total_vol):
        return cls.get_sum(population, objects, total_vol) / len(population)

    @classmethod
    def get_range(cls, population, objects, total_vol):
        return cls.get_maximum(population, objects, total_vol) - cls.get_minimum(population, objects, total_vol)

    @classmethod
    def _get_stationary(cls, data, objects, total_vol):
        aux = ""
        stationary = []
        for generation, population in enumerate(data):
            max_chromosome = Population.get_fittest_chromosome(population, objects, total_vol)
